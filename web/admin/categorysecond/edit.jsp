                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="panel panel-danger">
                        ...
                        <div class="panel-body">
                            <form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/adminCategorySecond_<s:if test="csid == null">add</s:if><s:else>edit</s:else>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="csid" value="<s:property value="csid"/>"/>
                                <div class="form-group">
                                    <label>二级列表名称</label>
                                    <input type="text" name="csname" value="<s:property value="csname"/>" id="userAction_save_do_logonName" class="bg"/>
                                </div>
                                <div class="form-group">
                                    <label>对应一级列表名称</label>
                                </div>
                                <div class="form-group">
                                <select name="category.cid">
                                    <s:iterator var="c" value="clist">
                                        <s:if test="category.cid==#c.cid">
                                            <option value="<s:property value="#c.cid"/>" selected="selected">
                                                <s:property value="#c.cname"/></option></s:if>
                                        <s:else>
                                ...
                                <input type="submit" name="sub3"  class="btn btn-danger" value="提交"/>
                               <input type="button"  class="btn btn-danger" value="退出" onclick="history.go(-1)"/>
                            </form>
                        </div>
                    </div>
                </div>
